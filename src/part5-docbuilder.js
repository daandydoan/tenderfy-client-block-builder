...
/* The definition a placed block renders from: its own copy once restyled,
   otherwise the library's. */
function itemBlockDef(it){
  if(it.blockDef) return it.blockDef;
  if(it.blockStyle){                                   // wrapper-only override
    const b = BLOCK_BY_ID[it.id], def = b && CUSTOM_BLOCK_DEF[b.p];
    if(def) return Object.assign({}, def, {blockStyle: it.blockStyle});
  }
  return null;
}
function docItemHtml(it, brand, cls, extra){
  ...
  if(it.t === 'element'){
    ...
    inner = b ? composeBlock(b, brand, content, itemBlockDef(it)) : '';
  }
...
(function(){
  ...
  function fillInspector(it){
    ...
    // Pulled from the admin (16-17 Sep 2026): where styling comes from, a way to the source block, and Arrange.
    $('bi-editBlock').style.display = it.t === 'element' ? 'none' : '';
    $('bi-sourceHint').textContent = it.t === 'element' ? 'Type and colour come from your brand - the renderer applies them. Add this element inside a block to style it.' : 'Layout and appearance below apply to this document only. Type and colour come from your brand. Edit the block to change every document that uses it.';
  ...
  /* ── Restyling a placed block ────────────────────────────────────────────
     ...
     here. The first edit takes a copy of the whole definition onto the document
  ...
  function ownBlockDef(it){
    if(!it.blockDef){
      ...
      it.blockDef = JSON.parse(JSON.stringify(def || {doc:[], blockStyle:{}}));
      // Fold in a wrapper-only override saved before elements were editable.
      if(it.blockStyle){ it.blockDef.blockStyle = it.blockStyle; delete it.blockStyle; }
    }
    return it.blockDef;
  }
  /* The definition this placement renders from, override or library. */
  function liveDefOf(it){ return itemBlockDef(it) || blockDefOf(it); }
  /* Flat list of the elements inside a block, with their row/col address. */
  function defElements(def){
    const out = [];
    (def && def.doc || []).forEach((row, r) => row.cols.forEach((col, c) => col.forEach((el, k) => out.push({el, r, c, k}))));
    return out;
  }
  let bsPick = 'block';   // 'block' or an element index, per selection

  const BS_ALIGN = [['left','Left'],['center','Centre'],['right','Right']];
  function fillBlockStyle(it){
    const host = $('bi-blockstyle'), def = liveDefOf(it);
    if(!def || !blockDefOf(it)){ host.innerHTML = ''; host.style.display = 'none'; return; }
    host.style.display = '';
    const els = defElements(def);
    if(bsPick !== 'block' && !els[bsPick]) bsPick = 'block';
    const edited = !!it.blockDef;
    const chips = `<div class="bs-chips">
        <button class="bs-chip ${bsPick==='block'?'on':''}" data-pick="block"><span class="ms">dashboard</span> Whole block</button>
        ${els.map((x,i) => `<button class="bs-chip ${bsPick===i?'on':''}" data-pick="${i}"><span class="ms">${PRIM_ICON[x.el.id]||'widgets'}</span> ${esc(BE_NAME[x.el.id]||x.el.id)}</button>`).join('')}
      </div>`;
    host.innerHTML = `
      <div class="ds-lbl" style="display:flex;align-items:center;gap:8px">Block style
        ${edited ? '<span class="bs-badge">edited</span>' : ''}
        ${edited ? '<button class="lbtn sm" style="margin-left:auto;padding:4px 10px" data-bs-reset>Reset</button>' : ''}</div>
    ...
    if(rst) rst.addEventListener('click', () => {
      delete it.blockDef; delete it.blockStyle; bsPick = 'block';
      markDirty(DB.doc); render(); fillBlockStyle(it);
      showToast("Back to the block's own style");
    });
  }
  const bsNum = (k, v, max) => `<input type="number" class="ds-num" data-bs="${k}" value="${v||0}" min="0" max="${max||200}">`;
  function blockFields(b){
    return `<div class="ds-row"><label>Padding</label><div class="ds-ctl">
        ${bsNum('padH', b.padH)}<span class="ds-u">H</span>${bsNum('padV', b.padV)}<span class="ds-u">V</span></div></div>
      <div class="ds-row"><label>Gap</label><div class="ds-ctl">${bsNum('sp', b.sp)}<span class="ds-u">px</span></div></div>
      ...
      <div class="ds-row"><label>Radius</label><div class="ds-ctl">${bsNum('rad', b.rad, 400)}<span class="ds-u">px</span></div></div>`;
  }
  function elementFields(el){
    const st = el.st || {};
    const isImg = el.id === 'image';
    const src = el.src === 'client' ? 'custom' : (el.src || 'placeholder');
    return `<div class="ds-row"><label>Padding</label><div class="ds-ctl">
        ${bsNum('padH', st.padH)}<span class="ds-u">H</span>${bsNum('padV', st.padV)}<span class="ds-u">V</span></div></div>
      <div class="ds-row"><label>Fill</label><div class="ds-ctl">
        <input type="color" data-bs="bg" value="${st.bg||'#ffffff'}">
        <label class="bi-tog" style="margin:0"><input type="checkbox" data-bs="bgOn"${st.bg?' checked':''}>on</label></div></div>
      <div class="ds-row"><label>Radius</label><div class="ds-ctl">${bsNum('rad', st.rad, 400)}<span class="ds-u">px</span></div></div>
      ${isImg ? `
      <div class="ds-lbl" style="margin-top:10px">Image</div>
      <div class="bg-modeseg" data-imgseg style="margin-bottom:8px">
        <button data-src="placeholder" class="${src==='placeholder'?'on':''}">Placeholder</button>
        <button data-src="custom" class="${src==='custom'?'on':''}">Custom Asset</button>
      </div>
      ${src === 'custom' ? `<div class="bg-img">
        ${el.img?`<img src="${el.img}" alt="">`:''}
        <label class="lbtn sm" style="width:100%;justify-content:center">
          <span class="ms">add_photo_alternate</span> ${el.img?'Replace image':'Choose an image'}
          <input type="file" data-bs-img accept="image/png,image/jpeg,image/webp,image/svg+xml" hidden></label>
        ${el.img?`<button class="lbtn sm" data-bs-imgclear style="width:100%;justify-content:center;margin-top:6px"><span class="ms">delete</span> Remove</button>`:''}
      </div>` : ''}` : `
      <div class="ds-row"><label>Align</label><div class="ds-ctl">
        <select class="ds-sel" data-bs="align"><option value="">Auto</option>
          ${[['left','Left'],['center','Centre'],['right','Right']].map(([v,n])=>`<option value="${v}"${st.align===v?' selected':''}>${n}</option>`).join('')}</select></div></div>`}`;
  }
  function wireBlockStyleFields(it, els){
    const host = $('bi-blockstyle');
    const targetOf = () => bsPick === 'block'
      ? (ownBlockDef(it).blockStyle = ownBlockDef(it).blockStyle || {})
      : (defElements(ownBlockDef(it))[bsPick].el.st = defElements(ownBlockDef(it))[bsPick].el.st || {});
    const touch = () => { markDirty(DB.doc); render(); fillBlockStyle(it); };
    host.querySelectorAll('[data-bs]').forEach(el => {
      ...
      el.addEventListener(ev, () => {
        ...
        if(k === 'bg'){ st.bgVis = true; st.bgOn = true; }
        touch();
      });
    });
    host.querySelectorAll('[data-imgseg] button').forEach(b => b.addEventListener('click', () => {
      defElements(ownBlockDef(it))[bsPick].el.src = b.dataset.src; touch();
    }));
    const file = host.querySelector('[data-bs-img]');
    if(file) file.addEventListener('change', ev => {
      const f = ev.target.files && ev.target.files[0]; if(!f) return;
      if(f.size > 1.5 * 1024 * 1024){ showToast('Pick an image under 1.5MB'); ev.target.value=''; return; }
      const rd = new FileReader();
      rd.onload = () => { const e2 = defElements(ownBlockDef(it))[bsPick].el; e2.img = rd.result; e2.src = 'custom'; touch(); showToast('Image added to this placement'); };
      rd.readAsDataURL(f); ev.target.value = '';
    });
    const clr = host.querySelector('[data-bs-imgclear]');
    if(clr) clr.addEventListener('click', () => { defElements(ownBlockDef(it))[bsPick].el.img = ''; touch(); });
  }
  ...
  // Arrange the selected block within this document (same ops as the canvas tools / drag)
  const selIdx = () => items().findIndex(x => x.k === selK);
  $('bi-up').addEventListener('click', () => { const i = selIdx(); if(i > 0){ const [m] = items().splice(i,1); items().splice(i-1,0,m); markDirty(DB.doc); render(); } });
  $('bi-down').addEventListener('click', () => { const i = selIdx(); if(i >= 0 && i < items().length-1){ const [m] = items().splice(i,1); items().splice(i+1,0,m); markDirty(DB.doc); render(); } });
  $('bi-dup').addEventListener('click', () => { const i = selIdx(); if(i < 0) return; const c = JSON.parse(JSON.stringify(items()[i])); c.k = ++uid; items().splice(i+1,0,c); selK = c.k; markDirty(DB.doc); render(); renderInspector(); showToast('Duplicated: ' + instLabel(c)); });
  $('bi-rm').addEventListener('click', () => { const i = selIdx(); if(i < 0) return; items().splice(i,1); selK = null; markDirty(DB.doc); render(); renderInspector(); showLeftTab('blocks'); });
  // Edit the source block: leave through the unsaved-changes guard, then open it in the Block Builder
  $('bi-editBlock').addEventListener('click', () => { const it = curItem; if(!it || it.t === 'element') return; exitEditor(DB.doc, null, () => { dbClose(); beOpenExisting(it.id); }, window.dbSave); });
