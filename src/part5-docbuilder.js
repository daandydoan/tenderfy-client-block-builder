...
(function(){
    ...
    w.querySelectorAll('.bg-reg').forEach(el => {
      ...
      el.querySelectorAll('[data-f]').forEach(inp => {
        ...
        inp.addEventListener(ev, () => {
          ...
          else if(f==='gradOn'){ r.gradOn=inp.checked; renderBgRegions(); }
  ...
  function fillInspector(it){
    ...
    const wm = s.wMode||'fill'; $('bs-wmode').value = wm; $('bs-wpx').value = s.wPx||480; $('bs-wpx').disabled = (wm !== 'fixed');
    const hm = s.hMode||'auto'; $('bs-hmode').value = hm; $('bs-hval').value = s.hVal||120; $('bs-hval').disabled = (hm==='auto'||hm==='hug');
  ...
  (function(){
    ...
    M.addEventListener('change', () => { if(!curItem) return; curItem.style.wMode = M.value; V.disabled = (M.value !== 'fixed'); markDirty(DB.doc); render(); });
  ...
  (function(){
    ...
    const upd = () => { if(!curItem) return; curItem.style.hMode = M.value; V.disabled = (M.value==='auto'||M.value==='hug'); curItem.style.hVal = Math.max(0,Math.round(+V.value||0)); markDirty(DB.doc); render(); };
